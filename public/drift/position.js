...
/* SEED RETENTION IS NOT ENFORCED HERE, AND THAT IS DELIBERATE.
 *
 * A SEED_TETHER_MIN of 0.414 shipped briefly, justified as the measured bge-m3
 * anisotropy — the mean cosine between two unrelated phrases. It was retracted
 * for three independent reasons, any one of which is disqualifying:
 *
 *  1. NOT REPRODUCIBLE HERE. Nothing in this repository computes 0.414. It
 *     comes from an external run described in a design document, and citing it
 *     as measured was the exact discipline failure this project keeps writing
 *     commit messages about.
 *  2. CONTRADICTED BY OUR OWN DATA. The walk spike recorded visibly abandoned
 *     candidates at 0.407-0.532 for the seed `public transit` — `yard twinkle
 *     lights` at 0.438, `lawn jenga party` at 0.414 exactly. A cutoff that
 *     admits the documented counterexamples proves nothing.
 *  3. IT KILLED LOCAL DEVELOPMENT. dev-fake-ai.ts's pseudo-embeddings score
 *     -0.19 to 0.20 against a seed, so DEV_FAKE_AI=1 — the documented offline
 *     path in the README — rendered a permanently empty surface. Measured: 0 of
 *     6 candidates passed.
 *
 * What is true, and measured over the shipped 2D loop across 277 surfaced
 * cards: retention holds BY CONSTRUCTION, because every candidate is generated
 * from the seed. tetherSeed ran min 0.377, p50 0.545, max 0.788
 * (docs/measurements/2026-08-23-production-parity-and-retention.md). That is a
 * property the pool has, not an invariant this module enforces, and the
 * difference is now stated rather than papered over.
 *
 * Enforcing it needs a non-circular relevance instrument. Workstream B looked
 * for one three times and returned a null result each time. Until one exists,
 * claiming the invariant is worse than admitting its absence. */
...
/** Unseen candidates within `radius` of the position. Supply and nextCard must
 *  apply the SAME filters or they disagree about whether anywhere is empty —
 *  cycle 1's blocker. Both now filter on reach and seen, and nothing else. */
