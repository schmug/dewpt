...
describe("draw", () => {
  ...
  it("serves fresh (post-invalidation) candidates before stale ones", () => {
    ...
    core.setParams({ dewpoint: 0.9 }, 2000); // invalidates
  ...
  it("keeps serving stale candidates after invalidation (old pool never stalls)", () => {
    ...
    core.setParams({ dewpoint: 0.9 }, 2000);
...
describe("invalidation", () => {
  it("dewpoint or altitude changes zero the fresh depth but keep the total", () => {
    const core = new PoolCore();
    core.addCandidates("w0a0", entries(["a", "b"]), 1000);
...
describe("persistence round-trip", () => {
  it("serialize/rehydrate preserves pool, anchors, exclude order, evaporated and invalidation stamp", () => {
    const a = new PoolCore();
    a.setSeedEmbedding([1, 0, 0]);
    a.setParams({ dewpoint: 0.7 }, 1000);
    a.addCandidates("w1a0", entries(["kept word"]), 2000);
    ...
    const b = new PoolCore(a.serialize());
    expect(b.depths().w1a0.total).toBe(1);
    expect(b.depths().w1a0.fresh).toBe(0); // pin at t=4000 invalidated the t=2000 candidate
